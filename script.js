...
(function () {
  ...
  if (leadForm) {
    leadForm.addEventListener("submit", function (e) {
      ...
      // In-app browsers (Instagram, FB) and popup blockers can swallow the new
      // tab — fall back to navigating this one so the lead never gets lost.
      // Note: passing "noopener" would make window.open return null on success
      // too, so the opener reference is severed manually instead.
      var win = window.open(url, "_blank");
      if (win) {
        try { win.opener = null; } catch (err) { /* cross-origin — harmless */ }
      } else {
